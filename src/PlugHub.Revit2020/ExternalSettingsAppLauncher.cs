...
{
    internal sealed class ExternalSettingsAppLauncher
    {
        private const string SettingsAppFileName = "PlugHub.SettingsApp.exe";
        ...
        public bool TryLaunch(string configDirectory, out string diagnostic)
        {
            ...
            var appPath = CandidateDirectories(configDirectory)
                .Select(directory => Path.Combine(directory, SettingsAppFileName))
                .FirstOrDefault(File.Exists);

            if (string.IsNullOrWhiteSpace(appPath))
            {
                diagnostic = "External settings app was not found: " + SettingsAppFileName;
                return false;
            }
        ...
        private static IEnumerable<string> CandidateDirectories(string configDirectory)
        {
            ...
            AddDirectory(directories, Path.GetDirectoryName(typeof(ExternalSettingsAppLauncher).Assembly.Location));
