...
设置页保存配置时不执行远端仓库刷新、程序集加载或运行时刷新。打开设置窗口只读取本地仓库缓存；只有用户在仓库页显式选择“浏览仓库插件包”或“检查更新”时才访问远端仓库。仓库访问使用 GitHub/Gitee HTTP archive 下载 zip 并替换本地 `repository-cache`，普通用户不需要安装 Git；缓存递归扫描包清单，安装和更新只是复制选中插件的单模块清单和必要文件到 `packages`，卸载只删除 `packages` 中对应已安装目录。新增插件、卸载插件和 Ribbon 结构变化需要重启 Revit 后重新渲染；更新未被 Revit 占用的业务 DLL 时直接替换 `packages` 文件，不写入待重启状态。如果 DLL 被占用，PlugHub 会先改写清单让模块不再被发现，并把删除或替换文件的动作记录为下次启动前执行的待处理操作。
